...
- `ARK_TTS_MODEL` / `ARK_TTS_VOICE`：切换 Ark 语音模型与音色
- `ARK_VIDEO_MODEL`：切换 Ark 视频模型
- `EXPORT_ROOT`：本地导出目录
...
- 默认文本模型 provider 现在建议使用 `ark`；分享稿、视频脚本这类聊天生成会走 `chat/completions`
- 默认 `AI_TTS_PROVIDER="none"`，视频会优先保证出片；如果你要让配音也走 Ark，可切到 `AI_TTS_PROVIDER="ark"`
- 默认 `AI_VIDEO_CLIP_PROVIDER="ark"`，视频片段生成会优先走 Ark 的视频任务接口；如需关闭可设为 `none`
